...
int main (int argc, char **argv){
    ...
    if( argc != 3 ){
    cerr << "Usage: h5_to_partpix hdf5file healpix_mapname" << endl;
    return 1;
    }
    ...
    return 1;
      ...
      */
      ...
      if( map_order > 13 ){
          cerr << "Error, the order > 13 and so we can't convert this to Healpix." << endl;
          return 1;
      }
      
      Healpix_Map<double> outMap;
      if( map_scheme == 1 ){
          outMap = Healpix_Map<double>( map_order, NEST );
      }
      else{
          outMap = Healpix_Map<double>( map_order, RING );
      }
      for( unsigned int i=1; i<ds1size; ++i ){
        outMap[map_pix[i]] = map_data[i];
      }
